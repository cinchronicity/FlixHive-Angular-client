...
export class FetchApiDataService {
  ...
  public getFavoriteMovies(): Observable<any> {
    ...
    // Get user info to extract favorite movie IDs
    return this.http
      .get(apiUrl + `/users/${username}`, {
        headers: new HttpHeaders({
          Authorization: 'Bearer ' + token,
      ...
      .pipe(
        map((response: any) => {
          // Extract favorite movie IDs from user object
          const userData = this.extractResponseData(response);
          const favoriteMovieIds = userData.favoriteMovies || [];

          // Return the array of movie IDs
          // Note: These are just IDs, not full movie objects
          // The frontend will need to match these with the full movies list
